...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const cr = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: 'claude-opus-4-8', max_tokens: 2000, system: MUSIC_SYSTEM, messages: [{ role: 'user', content: userText }] })
    });
    ...
    let out;
    try { const s = raw.indexOf('{'), e = raw.lastIndexOf('}'); out = JSON.parse(raw.slice(s, e + 1)); }
    catch (err) { return res.status(502).json({ error: 'Could not parse the music prompt.', detail: raw.slice(0, 400) }); }
