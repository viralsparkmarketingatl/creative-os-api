...
- PHOTOGRAPHS ARE ALWAYS IN FULL COLOR: any real photograph in the post MUST be rendered in natural, vivid FULL COLOR — NEVER black-and-white, grayscale, sepia, monochrome, or desaturated — UNLESS the brand kit EXPLICITLY states photos must be monochrome. This matters most for minimalist/monochrome BRANDS: a black-and-white DESIGN system (type, logo, layout) still uses COLOR photography. Every page's edit prompt MUST explicitly say the photo is in full natural color so the renderer does not desaturate it to match the design.
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // user's own photos to FEATURE as the subject/product in this post (optional, per batch item)
    const subjectImages = (Array.isArray(body.subjectImages) && body.subjectImages.length) ? body.subjectImages.filter(Boolean) : [];
    ...
    // the user's own subject/product photos to feature — added AFTER the style refs
    subjectImages.slice(0, 6).forEach(u => { const p = parseDataUrl(u); content.push({ type: 'image', source: { type: 'base64', media_type: p.media, data: p.b64 } }); });
    content.push({ type: 'text', text:
      'TOPIC / BRIEF: ' + (brief || '(none — use the blog)') +
      (blogText ? ('\n\nBLOG POST CONTENT (source material):\n' + blogText) : '') +
      (brandBlock ? ('\n\nBRAND KIT — every page MUST follow this exactly:\n' + brandBlock) : '') +
      (subjectImages.length ? ('\n\nSUBJECT PHOTOS TO FEATURE: the LAST ' + subjectImages.length + ' image(s) above are REAL photos the user wants FEATURED as the hero subject/product in this post. Recreate the brand STYLE + layout from the FIRST reference image, but make THESE real photos the main subject — cleanly composited on-brand (e.g. sticker cut-out / hero placement), NOT an invented subject. Each page prompt MUST explicitly instruct the renderer to feature the provided real photo(s) as the subject.') : '') +
    ...
    const renderRefs = refImages.concat(subjectImages); // GPT Image 2 edits see the style refs + the real subject photos
    const settled = await Promise.allSettled(plan.map((pg, idx) =>
      genImage(pg.prompt, renderRefs, size, quality, oKey).then(b64 => ({
