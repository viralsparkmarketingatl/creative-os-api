...
// HARDCODED (no env override so a stale SUNO_API_BASE can't break it):
// v2 unified music (all 6 engines) is ONLY on api.apiframe.ai; v1 per-engine + upload/extend + fetch on api.apiframe.pro.
const V2_BASE = 'https://api.apiframe.ai';
const V1_BASE = 'https://api.apiframe.pro';
  ...
  try {
    ...
    if (action === 'generate') {
      ...
      if (model === 'suno' && body.refAudio) {
        ...
        const upR = await fetch(V1_BASE + '/suno-upload', { method: 'POST', headers: { Authorization: key }, body: form });
        ...
        const exR = await fetch(V1_BASE + '/suno-extend', { method: 'POST', headers: { Authorization: key, 'Content-Type': 'application/json' }, body: JSON.stringify(exPayload) });
    ...
    if (action === 'status') {
      ...
      let r;
      if (body.poll === 'v1') {
        // Suno extend/upload flow uses the v1 (.pro) poll endpoint
        r = await fetch(V1_BASE + '/fetch', { method: 'POST', headers, body: JSON.stringify({ task_id: id }) });
      } else {
        r = await fetch(V2_BASE + '/v2/jobs/' + encodeURIComponent(id), { method: 'GET', headers });
      }
      const txt = await r.text(); let d; try { d = JSON.parse(txt); } catch (e) { d = {}; }
